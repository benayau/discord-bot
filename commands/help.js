...
exports.run = (client, message, args) => { //Collecting info about command
  ...
  var help2 = new Discord.RichEmbed() //Embed with help
    .setColor(0xf4f142)
  var help3 = new Discord.RichEmbed() //Embed with help
    .setColor(0xf4f142)
  ...
  if (args.length == 0) { // If no args
    for (var i in commandsFile) { // In commandsFile
      if (commandsFile[i].group === "User") { // If type is User
        ...
        if (cmdFound >= 50) {
          cmdFound++; //++Commands found
          help3.addField(commandsFile[i].name, `**Description:** ${commandsFile[i].desc}\n**Usage:** \`\`${pref + commandsFile[i].usage}\`\``); //Adding fields with help
        }
      }
    }
    ...
    switch (cmdFound) {
      ...
      case cmdFound >= 50:
        help3.setFooter(`Now showing only user commands. To view another group do ${pref}help (group|command)`); //Footer
      break;
    ...
    if (cmdFound >= 50) {
      message.author.send(help3) //Sending to author
    }
    message.channel.send({
      embed: {
        "description": "Sent you DM with help :ok_hand: ",
        "color": 5164111
      }
